...
shine_global_config *shine_initialise(shine_config_t *pub_config)
{
  ...
  // MALLOC_CAP_DIRAM (upstream's original cap here) doesn't exist on the
  // ESP32-S3 - it's an original-ESP32-only concept (RAM dual-mapped into
  // both the D- and I-bus address spaces) this chip's esp_heap_caps.h
  // doesn't define at all. MALLOC_CAP_8BIT|MALLOC_CAP_INTERNAL is the
  // portable equivalent of what upstream actually wanted here: internal
  // (never SPIRAM) RAM, byte-addressable.
  // Upstream never checked any of the heap_caps_malloc() calls below for
  // NULL except this very first one - on a board with plenty of free
  // RAM that's merely sloppy, but on this no-PSRAM board running close
  // to its limit (see speaker.cpp's mic_start_recording() comment) it's
  // a real, easy-to-hit crash: a single failed sub-allocation among the
  // dozen below used to mean either an immediate NULL-pointer write
  // right here, or a config->l3_enc[x][y]/mdct_freq[x][y]/l3loop garbage
  // pointer silently carried forward into shine_subband_initialise() /
  // shine_mdct_initialise() / shine_loop_initialise() a few lines down,
  // corrupting unrelated heap memory instead of failing cleanly - either
  // way, the device panics and reboots rather than mic_start_recording()
  // getting a clean NULL back to report as "encoder init failed". Every
  // allocation below is now checked, freeing whatever already succeeded
  // and returning NULL on the first failure - the well-behaved counterpart
  // to the one at the top of this function upstream already had.
  config = heap_caps_malloc(sizeof(shine_global_config), MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
  if (config == NULL)
    return config;
  memset(config, 0, sizeof(*config)); // so the alloc_failed cleanup below only frees pointers actually assigned so far, never uninitialized heap garbage
  ...
  for (x = 0; x < MAX_CHANNELS; x++) {
      for (y = 0; y < MAX_GRANULES; y++) {
        ...
        if (!config->l3_enc[x][y] || !config->mdct_freq[x][y]) goto alloc_failed;
      }
  }
  printf("l3loop struct: %d\n", sizeof(l3loop_t));
  config->l3loop = heap_caps_malloc(sizeof(l3loop_t), MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL); // see comment above
  if (!config->l3loop) goto alloc_failed;
  memset(config->l3loop, 0, sizeof(*config->l3loop));
  ...
  if (!config->l3loop->xrsq || !config->l3loop->xrabs) goto alloc_failed;
...
alloc_failed:
  // Free whatever succeeded before the failing allocation above - config
  // and config->l3loop were both memset() to 0 right after their own
  // allocations, so every pointer field here is either a real allocation
  // or a guaranteed NULL (free(NULL) is a no-op), never uninitialized
  // heap garbage from before this function touched it.
  if (config->l3loop) {
...
unsigned char *shine_encode_buffer(shine_global_config *config, int16_t **data, int *written)
{
  ...
  // Upstream also had `if (config->wave.channels == 2) config->buffer[1] = data[1];`
  // here - dead code speaker.cpp never exercises (config->wave.channels
  // is always PCM_MONO/1), but now that types.h's MAX_CHANNELS is 1 for
  // this vendored copy, `config->buffer` only has one element:
  // `config->buffer[1]` would be a real out-of-bounds write if this ever
  // did run (the compiler now flags exactly that - see
  // types.h's MAX_CHANNELS comment). Removed rather than left as an
  // unreachable landmine.
...
unsigned char *shine_encode_buffer_interleaved(shine_global_config *config, int16_t *data, int *written)
{
  ...
  // See shine_encode_buffer()'s comment just above - same removed dead
  // (and, with MAX_CHANNELS=1, unsafe) stereo branch.
...
void shine_close(shine_global_config *config) {
  // Upstream only ever freed `config` itself here, leaking every
  // heap_caps_malloc() shine_initialise() made for l3_enc/mdct_freq/
  // l3loop/xrsq/xrabs (see that function's own comment) - harmless on a
  // board with RAM to spare, but on this one a second mic_start_recording()
  // call would find less and less free heap each time, eventually
  // hitting the same crash the alloc_failed path there now guards
  // against instead of ever cleanly failing. Mirrors the frees
  // shine_initialise()'s own alloc_failed cleanup does.
  int x, y;
  if (config->l3loop) {
    heap_caps_free(config->l3loop->xrsq);
    heap_caps_free(config->l3loop->xrabs);
