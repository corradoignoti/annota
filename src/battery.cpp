...
#elif defined(BOARD_EPAPER_397)
    // TODO(board-397): this board has no GPIO battery-ADC pin at all -
    // battery voltage would need an AXP2101 register read (see battery.h's
    // top comment), not analogReadMilliVolts(). Stubbed since wiring that
    // up is a feature addition beyond this port's scope.
    return 0;
#endif
}
...
uint8_t battery_read_percent() {
    ...
    return BATTERY_PERCENT_UNKNOWN; // see battery.h's top comment
#else
