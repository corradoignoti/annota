...
// delete) at "/", plus a web version of the on-screen Settings view (WiFi/
// clock status, SD info, Reconnect WiFi, Delete WiFi Setup) at "/settings",
// served over WiFi on port 80. Each request that touches the SD card
// borrows the shared SPI peripheral from touch for the duration of that one
// request (see storage.h's sd_begin() and display.h's
